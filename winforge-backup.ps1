...
function Get-XmlConfig {
    ...
    try {
        ...
        if ($isEncrypted) {
            Write-SystemMessage -msg1 "Encrypted configuration detected. Please enter password."
            
            # Create password prompt form
            ...
            if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
                try {
                    ...
                    if (Convert-SecureConfig -FilePath $Path -IsEncrypting $false -Password $password.Text) {
                ...
                throw "Password entry cancelled by user."
            }
        }
...
# Add these functions from secure-config.ps1
...
function Convert-SecureConfig {
    # Copy the entire Convert-SecureConfig function from secure-config.ps1
